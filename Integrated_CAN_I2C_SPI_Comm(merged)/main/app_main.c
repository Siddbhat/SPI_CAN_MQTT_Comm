...
void app_main(void)
{
    ...
    while (1)  
    {
        ...
        if (size > 0) { 		
			LED_INTENSITY = recvbuf[0];  // First byte
   	        LED_INTENSITY=LED_INTENSITY << 8 | recvbuf[1]; // Combine first and second byte
...
//	        printf(" the I2c msg %s\n", push_msg);
          ...
          #ifdef SPI_ENABLE
    t.length = 10 * 8; // 10 bytes * 8 bits per byte = 80 bits
    t.rx_buffer = recvbuf; // Pointer to the receive buffer
    if(spi_slave_transmit(HSPI_HOST, &t, portMAX_DELAY)==ESP_OK)
    {
	   Led_inten = recvbuf[0]; // First byte
   	   Led_inten=Led_inten << 8 | recvbuf[1];   // Combine first and second byte
	   sprintf(push_msg, "%04x", Led_inten);
	   Led_inten = Led_inten / 252;
   	 //  int dac_value = (Led_inten * 255) / 255;  // Scale the 8-bit value to DAC range
        // Set the DAC output value on GPIO 25 (DAC2)
...
void i2c_slave_init() {
    ...
    i2c_config_t conf_slave = {
        ...
        .sda_pullup_en = GPIO_PULLUP_DISABLE,  // disables internal pull-ups ( external pull-ups are used)
...
static void mqtt_app_start(void)
{
    ...
    char line[128];  // Buffer to store the broker URL

    if (strcmp(mqtt_cfg.broker.address.uri, "FROM_STDIN") == 0) // Check if the broker URL is set to "FROM_STDIN"
    {
        int count = 0; // Counter to track the number of characters read
        printf("Please enter url of mqtt broker\n"); // Prompt user for the MQTT broker URL
        
        // Read characters from stdin one by one
        while (count < 128)  // Ensure we do not exceed the buffer size
        { 
            int c = fgetc(stdin);  // Read a character from stdin
            if (c == '\n')  // Check if the character is a newline (end of input)
            {
                line[count] = '\0';  // Null-terminate the string
                break;
            }
            else if (c > 0 && c < 127) // Ensure the character is valid ASCII
            {
                line[count] = c;  // Store the character in the buffer
                ++count;
            }
            vTaskDelay(10 / portTICK_PERIOD_MS);   // Small delay to prevent busy-waiting
        }
        mqtt_cfg.broker.address.uri = line; // Set the broker URL in the MQTT configuration
        printf("Broker url: %s\n", line);  // Print the broker URL to confirm
    }
    } 
    else
    {
        ESP_LOGE(TAG, "Configuration mismatch: wrong broker url");  // Error message if URL is not "FROM_STDIN"
        abort();
    }
#endif /* CONFIG_BROKER_URL_FROM_STDIN */
  
   
    esp_mqtt_client_start(client); // Start the MQTT client with the configured broker URL
}

void SPI_Init(void)
{
	spi_bus_config_t buscfg = {
        .mosi_io_num = GPIO_MOSI,  // GPIO pin for Master-Out Slave-In (MOSI)
        .miso_io_num = GPIO_MISO,  // GPIO pin for Master-In Slave-Out (MISO)
        .sclk_io_num = GPIO_SCLK,   // GPIO pin for SPI clock (SCLK)
    ...
    spi_slave_interface_config_t slvcfg = {
        ...
        .spics_io_num = GPIO_CS,              // GPIO pin for SPI Chip Select (CS)
        .queue_size = 3,                      // Number of transactions to queue
...
// Converts an integer to a string with leading zeros if needed
int intToStr(int x, char str[], int d) 
{ 
	int i = 0; 
	while (x) { 
...
{ 
    ...
    // Convert integer part to string and store in res
	int i = intToStr(ipart, res, 0); 

    // If there are decimal places to be included
	if (afterpoint != 0) { 
		res[i] = '.'; // add dot 
